...
public class StandardSettingsConfig implements SpeedrunConfig {
...

    {
        ...
        this.register("fancyGraphics", "options.video", Option.GRAPHICS, options -> options.fancyGraphics ? 1 : 0);
        ...
        this.register("gamma", "options.video", new DoubleOption("options.gamma", 0.0, 5.0, 0.0f, options -> options.gamma, (options, value) -> options.gamma = value, (options, option) -> (int) (option.get(options) * 100.0) + "%"));
        ...
        }, option -> new ButtonWidget(0, 0, 120, 20, option.getText(), button -> MinecraftClient.getInstance().openScreen(new LanguageOptionsScreen(MinecraftClient.getInstance().currentScreen, options, MinecraftClient.getInstance().getLanguageManager()))));
        ...
        for (KeyBinding keyBinding : keyBindings) {
            this.register(new KeyBindingStandardSetting("key_" + keyBinding.getId(), keyBinding.getCategory(), keyBinding));
        }
    ...
    private StringOptionStandardSetting register(String id, String category, Function<GameOptions, String> getter, BiConsumer<GameOptions, String> setter, Function<StringOptionStandardSetting, String> getText, Function<StringOptionStandardSetting, AbstractButtonWidget> createMainWidget) {
    ...
    @Override
    public @Nullable Predicate<InputUtil.KeyCode> createInputListener() {
        return key -> {
            if (this.focusedKeyBinding != null) {
                if (key.getCategory() == InputUtil.Type.KEYSYM && key.getKeyCode() == GLFW.GLFW_KEY_ESCAPE) {
